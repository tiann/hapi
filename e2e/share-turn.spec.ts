...
test('localizes the share dialog and exported badge in Chinese', async ({ page }) => {
    ...
    await expect(dialog.getByText('分享会话', { exact: true })).toBeVisible()
    await expect(dialog.getByText('Generated by HAPI', { exact: true })).toBeVisible()
    await expect(dialog.getByRole('button', { name: '取消' })).toBeVisible()
    await expect(dialog.getByRole('button', { name: '复制图片' })).toBeVisible()
