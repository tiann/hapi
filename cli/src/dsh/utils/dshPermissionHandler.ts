...
export class DshPermissionHandler extends BasePermissionHandler<PermissionResponseMessage, void> {
    private readonly pendingBackendRequests = new Map<string, PermissionRequest>();

    constructor(
        session: ApiSessionClient,
        private readonly backend: AgentBackend,
        private readonly getPermissionMode: () => DshPermissionMode | undefined
    ) {
        super(session);
    ...
    private handlePermissionRequest(request: PermissionRequest): void {
        ...
        logger.debug(`[DSH] Permission request queued for ${toolName} (${request.id})`);
    }
    ...
    private async autoApprove(
        ...
        logger.debug(`[DSH] Auto-approved ${toolName} (${request.id}) mode=${decision}`);
    }
    ...
    protected async handlePermissionResponse(
        ...
        if (pendingRequest) {
            ...
            logger.debug('[DSH] Permission response missing backend request', response.id);
        }
        ...
        logger.debug(`[DSH] Permission ${response.approved ? 'approved' : 'denied'} for ${pending.toolName}`);
    ...
    protected handleMissingPendingResponse(response: PermissionResponseMessage): void {
        logger.debug('[DSH] Permission response received for unknown request', response.id);
    }
