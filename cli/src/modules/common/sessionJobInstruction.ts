...
export const SESSION_JOB_INSTRUCTION = [
    ...
    'Prefer supervised CLI for process-shaped work (idle agents cannot heartbeat):',
    'hapi job run "$HAPI_SESSION_ID" <job-key> --label <text> -- <cmd>…',
    '(auto-heartbeats + completed/failed on exit).',
    'Manual path: MCP tool session_job (aliases: mcp__hapi__session_job,',
    'hapi_session_job, functions.hapi__session_job) action=set, then action=update',
    'every ~10m from a self-heartbeating wrapper — never set-once and walk away.',
