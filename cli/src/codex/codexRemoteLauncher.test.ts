...
describe('codexRemoteLauncher', () => {
    ...
    it('preserves the explicitly selected settings after materializing a HAPI fork', async () => {
        ...
        const { session, getModel } = createSessionStub(['first message'], {
            ...createMode(), model: 'gpt-5.3-codex', modelReasoningEffort: 'high', serviceTier: 'fast'
        });
        session.sessionId = 'thread-source';
        session.codexForkRequest = { sourceThreadId: 'thread-source' };
        ...
        expect(harness.startTurnParams[0]).toMatchObject({
            ...
            collaborationMode: { settings: { model: 'gpt-5.3-codex', reasoning_effort: 'high' } },
