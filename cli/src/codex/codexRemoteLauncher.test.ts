...
function createSessionStub(opts?: { messages?: string[]; messageSeqs?: number[]; forkSessionId?: string | null; forkHistory?: unknown[] }) {
    const messages = opts?.messages ?? ['hello from launcher test'];
    const queue = new MessageQueue2<EnhancedMode>((mode) => JSON.stringify(mode));
    messages.forEach((message, index) => {
        const messageSeq = opts?.messageSeqs?.[index] ?? null;
    ...
    const session = {
        ...
        forkHistory: opts?.forkHistory ?? undefined,
        thinking: false,
        getPermissionMode() {
