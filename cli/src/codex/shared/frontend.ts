...
export async function runSharedCodex(raw: SharedLaunchOptions): Promise<void> {
    ...
    if (options.existingSessionId) {
        ...
        if (session.metadata?.codexForkCleanup) throw new Error('Codex fork cleanup is pending');
        ...
        if (session.metadata?.codexForkRequest) pendingForkParams(session.metadata.codexForkRequest, options.resumeSessionId);
