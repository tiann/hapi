...
beforeEach(() => {
    vi.clearAllMocks(); state.run.mockReset(); state.existing = false; state.metadata = {};
...
describe('shared frontend execution ownership', () => {
    it.each(['cleanup', 'source', 'boundary'])('rejects invalid pending fork %s before runtime launch', async invalid => {
        ...
        if (invalid === 'boundary') state.metadata.codexForkRequest = { sourceThreadId: 'source', lastTurnId: 'a', beforeTurnId: 'b' };
        state.run.mockResolvedValueOnce(undefined);
        await expect(runSharedCodex({ existingSessionId: 'sid', startedBy: 'runner', workingDirectory: '/work',
            resumeSessionId: invalid === 'source' ? 'wrong' : 'source' })).rejects.toThrow(/fork/i);
