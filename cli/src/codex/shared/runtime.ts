...
export async function runSharedRuntime(options: SharedLaunchOptions, onReady?: (ready: RuntimeReady) => void, abortSignal?: AbortSignal): Promise<void> {
    ...
    const create = (method: 'thread/start' | 'thread/fork', params: Record<string, unknown>, parent?: SharedCodexRoot, initialOptions?: SharedLaunchOptions): Promise<SharedCodexRoot> => operation(async () => {
        ...
        try {
            ...
            if (initialOptions?.existingSessionId) {
                const metadata = root.session.getMetadata();
                if (!metadata?.codexForkRequest || metadata.codexForkCleanup) throw new Error('Pending Codex fork no longer exists');
                const expected = pendingForkParams(metadata.codexForkRequest, metadata.codexSessionId);
                if (method !== 'thread/fork' || expected.threadId !== params.threadId
                    || expected.lastTurnId !== params.lastTurnId || expected.beforeTurnId !== params.beforeTurnId) {
                    throw new Error('Codex fork source or boundary changed');
                }
            }
            ...
            if (method === 'thread/fork' && string(record(response.thread).id) === params.threadId) throw new Error('Codex fork returned its source');
            ...
            if (!nativeSucceeded && !isIndeterminateError(error)) {
                runtime.pendingCreations = runtime.pendingCreations?.filter(sid => sid !== root.session.sessionId); await persist();
                prepared.delete(root); await root.close(!initialOptions?.existingSessionId);
            }
    ...
    const after = (request: Envelope, response: Envelope, connection: string): Promise<void | Envelope[]> => operation(async () => {
        ...
        if (reservation) {
            ...
            if (request.method === 'thread/fork' && string(record(record(response.result).thread).id) === string(record(request.params).threadId)) {
                throw new Error('Codex fork returned its source');
            }
    ...
    startup = (async () => {
        ...
        const existing = options.existingSessionId
            ? await (await ApiClient.create()).getSession(options.existingSessionId) : undefined;
        const existingMetadata = existing?.metadata;
        if (existingMetadata?.codexForkCleanup) throw new Error('Codex fork cleanup is pending');
        if (existingMetadata?.codexForkRequest) {
            if (existing?.active) throw new Error('Pending Codex fork is already active');
            root = await create('thread/fork', { ...launch.threadParams, cwd: launch.cwd,
                ...pendingForkParams(existingMetadata.codexForkRequest, options.resumeSessionId) }, undefined, options);
        } else if (options.resumeSessionId) {
            if (existingMetadata?.codexSessionId && existingMetadata.codexSessionId !== options.resumeSessionId) {
                throw new Error('Codex session binding changed before resume');
            }
