...
export class SharedCodexRoot {
    ...
    private pendingFork: SessionBootstrapResult['metadata']['codexForkRequest'];

    constructor(readonly bootstrap: SessionBootstrapResult, private readonly host: RootHost) {
        this.session = bootstrap.session;
        this.pendingFork = structuredClone(bootstrap.sessionInfo?.metadata?.codexForkRequest);
    ...
    async bind(threadId: string, response: Record<string, unknown>, subscribe: boolean): Promise<void> {
        if (this.pendingFork) {
            pendingForkParams(this.pendingFork, this.session.getMetadata()?.codexSessionId);
            if (threadId === this.pendingFork.sourceThreadId) throw new Error('Codex fork returned its source');
        }
        ...
        this.session.updateMetadata(metadata => ({ ...metadata, ...(this.pendingFork ? {} : { codexSessionId: threadId }), capabilities: {
    ...
    async activate(options: SharedLaunchOptions = {}): Promise<void> {
        ...
        if (this.pendingFork) {
            const request = this.pendingFork;
            // CAS retries must not erase a cleanup owner or a changed request.
            // Publish the distinct ID and consume the intent in the same write.
            this.session.updateMetadata(metadata => {
                if (metadata.codexForkCleanup || !isDeepStrictEqual(metadata.codexForkRequest, request)
                    || metadata.codexSessionId !== request.sourceThreadId || this.threadId === request.sourceThreadId) return metadata;
                return { ...metadata, codexSessionId: this.threadId, codexForkRequest: undefined };
            });
            const confirmed = await this.session.flushMetadata();
            const metadata = this.session.getMetadata();
            if (!confirmed || metadata?.codexSessionId !== this.threadId || metadata.codexForkRequest || metadata.codexForkCleanup) {
                throw new Error('Codex fork binding was not confirmed');
            }
