...
async function fixture(pending?: Metadata['codexForkRequest']) {
    ...
    let metadata: Metadata = { path: directory, host: 'test', flavor: 'codex',
        ...(pending ? { codexSessionId: pending.sourceThreadId, codexForkRequest: pending } : {}) };
    const bootstrapMetadata = structuredClone(metadata);
    ...
    const session = {
        ...
        sendSessionDeath() {}, async flush() { return true; }, async flushMetadata() { return true; }, close() {}
...
describe('pending fork activation', () => {
    it('keeps the source anchor until settings succeed, then confirms the atomic child binding before admission', async () => {
        ...
        expect(f.metadata()).toMatchObject({ codexSessionId: 'source', codexForkRequest: request });
        vi.spyOn(f.root, 'initialSettings').mockRejectedValueOnce(new Error('settings failed'));
        await expect(f.root.activate()).rejects.toThrow('settings failed');
        expect(f.metadata()).toMatchObject({ codexSessionId: 'source', codexForkRequest: request });
        expect(f.rpc.has(RPC_METHODS.ForkConversation)).toBe(false);
        let release!: (confirmed: boolean) => void;
        const flush = vi.spyOn(f.root.session, 'flushMetadata').mockImplementationOnce(() => new Promise(resolve => { release = resolve; }));
        const activating = f.root.activate();
        await vi.waitFor(() => expect(flush).toHaveBeenCalledOnce());
        expect(f.metadata().codexSessionId).toBe('thread');
        expect(f.metadata().codexForkRequest).toBeUndefined();
        expect(f.rpc.has(RPC_METHODS.ForkConversation)).toBe(false);
        release(true); await activating;
        expect(f.rpc.has(RPC_METHODS.ForkConversation)).toBe(true);
    });
    it.each(['cleanup', 'boundary', 'unconfirmed'])('does not activate after %s wins the binding race', async race => {
        const f = await fixture({ sourceThreadId: 'source' });
        if (race === 'cleanup') f.root.session.updateMetadata(m => ({ ...m, codexForkCleanup: { sourceSessionId: 'parent', machineId: 'machine' } }));
        if (race === 'boundary') f.root.session.updateMetadata(m => ({ ...m, codexForkRequest: { sourceThreadId: 'source', lastTurnId: 'changed' } }));
        if (race === 'unconfirmed') vi.spyOn(f.root.session, 'flushMetadata').mockResolvedValue(false);
        await expect(f.root.activate()).rejects.toThrow(/fork/i);
        expect(f.rpc.has(RPC_METHODS.ForkConversation)).toBe(false);
        if (race !== 'unconfirmed') expect(f.metadata().codexForkRequest).toBeDefined();
    });
});
