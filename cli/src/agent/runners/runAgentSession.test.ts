...
vi.mock('@/claude/utils/startHappyServer', () => ({
    startHappyServer: vi.fn(async (_session: unknown, options: unknown) => {
        ...
        return {
            ...
            toolNames: ['change_title', 'display_image', 'list_peers', 'ping_peer', 'inspect_peer', 'session_job', 'skill_lookup'],
...
describe('runAgentSession', () => {
    ...
    it('enables skill lookup and injects its instruction only on the first prompt', async () => {
        ...
        expect(harness.bridgeArgs).toEqual([
            ...
            'http://127.0.0.1:1234',
            '--tools',
            'change_title,display_image,list_peers,ping_peer,inspect_peer,session_job,skill_lookup'
        ])
