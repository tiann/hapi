...
import type { OmpAgentEvent, OmpMessageUpdateEvent } from './types'
import { OmpAssistantMessageEventSchema } from './schemas'

/**
 * Accumulates OMP assistant-message text/thinking deltas into a single
 ...
 *
 * Same event shape as Pi (shared pi-agent-core AssistantMessageEvent).
 */
export class OmpMessageAccumulator {
    private active = false
    private text = ''
    private reasoning = ''
    private streamId = 'omp-stream'

    /**
     * Apply an OMP event to the accumulator.
     *
     ...
     */
    handleEvent(event: OmpAgentEvent): AgentMessage[] {
        if (event.type === 'message_start') {
            this.active = true
            this.text = ''
            this.reasoning = ''
            this.streamId = 'omp-stream'
            return []
        }

        if (event.type === 'message_update') {
            const updateEvent = event as OmpMessageUpdateEvent
            const rawAme = updateEvent.assistantMessageEvent
            if (!rawAme) return []
            const ameResult = OmpAssistantMessageEventSchema.safeParse(rawAme)
            if (!ameResult.success) {
                logger.debug(`[omp] Failed to parse assistantMessageEvent: ${ameResult.error.message.slice(0, 200)}`)
                return []
            }
            const ame = ameResult.data
            const streamId = ame.contentIndex?.toString() ?? 'omp-stream'
        ...
        // message ended without a clean `message_end` (partial streams,
        // or a stream that crashed mid-flight).
    ...
    private flush(): AgentMessage[] {
        ...
        // Reasoning comes before text in the OMP event sequence, so emit
