...
describe('shared Codex hub binding', () => {
    it.each(['source', 'pending', 'cleanup'])('rejects a shared fork response with %s identity/state', async invalid => {
        const { engine, create, rpc } = fixture()
        try {
            const source = create('source')
            rpc.forkConversation = async () => {
                const child = create('child', { forkedFrom: source.id,
                    codexSessionId: invalid === 'source' ? 'native-source' : 'native-child',
                    ...(invalid === 'pending' ? { codexForkRequest: { sourceThreadId: 'native-source' } } : {}),
                    ...(invalid === 'cleanup' ? { codexForkCleanup: { sourceSessionId: source.id, machineId: 'machine' } } : {}) })
                return { nativeSessionId: child.metadata!.codexSessionId!, sessionId: child.id }
            }
            expect(await engine.forkConversation(source.id, 'default')).toMatchObject({ type: 'error' })
