...
describe('Store V26→V27 migration: ACP usage columns', () => {
    it('upgrades a base V26 database before the usage scan', () => {
        const directory = mkdtempSync(join(tmpdir(), 'hapi-migration-v26-to-v27-'))
        ...
        try {
            ...
            db.exec(`
                ...
                INSERT INTO usage_events (
                    ...
                    'migration-v26-usage', 'stale', 1, 1, 'claude', 'delta', 1, 1, 0, 0
                );
                INSERT INTO usage_scan_state (session_id, message_epoch, last_seq)
                VALUES ('migration-v26-usage', 0, 1);
    ...
    it('rebuilds stale usage indexes when a v26 database already has the new columns', () => {
        const directory = mkdtempSync(join(tmpdir(), 'hapi-migration-v26-recovery-'))
        const dbPath = join(directory, 'test.db')
        let store: Store | undefined
        try {
            store = new Store(dbPath)
            const session = store.sessions.getOrCreateSession(
                'migration-v26-recovery',
                { path: '/tmp', host: 'test', flavor: 'claude' },
                null,
                'default'
            )
