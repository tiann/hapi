...
export function createSessionsRoutes(getSyncEngine: () => SyncEngine | null): Hono<WebAppEnv> {
    ...
    app.post('/sessions/:id/model-reasoning-effort', async (c) => {
        ...
        const flavor = sessionResult.session.metadata?.flavor ?? 'claude'
        if (flavor !== 'codex' && flavor !== 'opencode' && flavor !== 'dsh') {
            return c.json({ error: 'Model reasoning effort is only supported for Codex, OpenCode and DeepSeek Harness sessions' }, 400)
        }
        if (sessionResult.session.agentState?.controlledByUser === true) {
