...
function createApp(
    manager: Partial<HubPluginManager> | null,
    engine: Partial<SyncEngine> | null = null,
    marketplaceService?: PluginMarketplaceService,
    options?: Parameters<typeof createPluginsRoutes>[3]
) {
...
function createInstallPlanRouteApp(options: {
    manager: Partial<HubPluginManager>
    engine?: Partial<SyncEngine> | null
    marketplaceService?: PluginMarketplaceService | null
    ...
    const app = new Hono<WebAppEnv>()
    const routes = new Hono<WebAppEnv>()
    app.use('/api/*', createAuthMiddleware(secret))
    registerPluginInstallPlanAndMarketplaceRoutes(routes, {
        getPluginManager: () => options.manager as HubPluginManager,
        getSyncEngine: () => options.engine as SyncEngine | null ?? null,
        resolveMarketplaceService: () => options.marketplaceService ?? null,
        installPlanTempDir: options.installPlanTempDir,
        maxInstallPlans: options.maxInstallPlans,
        maxInstallPlanPackageBytes: options.maxInstallPlanPackageBytes,
        maxInstallPlanStorageBytes: options.maxInstallPlanStorageBytes
    })
...
describe('plugin admin routes', () => {
    ...
    it('rejects oversized direct package installs before inspection or Runner distribution', async () => {
        const online = makeMachine('runner-1', true)
        const calls: string[] = []
        const app = createApp({ listPlugins: () => [] } as never, {
            getMachineByNamespace: () => online,
        ...
        } as never, undefined, { maxPluginPackageBytes: 1 })
        ...
        expect(await response.json()).toEqual({ error: 'Plugin package is too large.' })
        expect(calls).toEqual([])
    })
    ...
    it('evicts install-plan package payloads from temp storage instead of keeping them in memory', async () => {
        const tempRoot = mkdtempSync(join(tmpdir(), 'hapi-plugin-install-plan-test-'))
        const firstPackage = makeTgzPackage('com.example.first')
        const secondPackage = makeTgzPackage('com.example.second')
        const installRequests: unknown[] = []
        try {
            const app = createInstallPlanRouteApp({
                manager: {
                    listPlugins: () => [],
                ...
                installPlanTempDir: tempRoot,
                maxInstallPlans: 10,
                maxInstallPlanStorageBytes: Math.max(firstPackage.body.contentBase64.length, secondPackage.body.contentBase64.length)
            })
            const headers = { authorization: `Bearer ${await token()}`, 'content-type': 'application/json' }

            const firstResponse = await app.request('/api/plugins/install-plan', {
                method: 'POST',
                headers,
                body: JSON.stringify(firstPackage.body)
            })
            expect(firstResponse.status).toBe(200)
            const firstPlan = await firstResponse.json() as { planId: string }
            expect(readdirSync(tempRoot)).toHaveLength(1)

            const secondResponse = await app.request('/api/plugins/install-plan', {
                method: 'POST',
                headers,
                body: JSON.stringify(secondPackage.body)
            })
            expect(secondResponse.status).toBe(200)
            const secondPlan = await secondResponse.json() as { planId: string }
            expect(readdirSync(tempRoot)).toHaveLength(1)

            const evictedResponse = await app.request(`/api/plugins/install-plan/${firstPlan.planId}/execute`, {
                method: 'POST',
                headers
            })
            expect(evictedResponse.status).toBe(404)

            const executeResponse = await app.request(`/api/plugins/install-plan/${secondPlan.planId}/execute`, {
                method: 'POST',
                headers
            })
            expect(executeResponse.status).toBe(200)
            ...
            expect((installRequests[0] as { contentBase64?: string }).contentBase64).toBe(secondPackage.body.contentBase64)
        } finally {
            firstPackage.cleanup()
            secondPackage.cleanup()
            rmSync(tempRoot, { recursive: true, force: true })
        }
    })

    it('keeps install-plan temp storage bounded under concurrent plan creation', async () => {
        const tempRoot = mkdtempSync(join(tmpdir(), 'hapi-plugin-install-plan-concurrent-test-'))
        const packages = [
            makeTgzPackage('com.example.concurrent-a'),
            makeTgzPackage('com.example.concurrent-b'),
            makeTgzPackage('com.example.concurrent-c')
        ]
        const installRequests: unknown[] = []
        try {
            const app = createInstallPlanRouteApp({
                ...
                installPlanTempDir: tempRoot,
                maxInstallPlans: 10,
                maxInstallPlanStorageBytes: Math.max(...packages.map((entry) => entry.body.contentBase64.length))
            })
            const headers = { authorization: `Bearer ${await token()}`, 'content-type': 'application/json' }

            const planResponses = await Promise.all(packages.map(async (packageFixture) => await app.request('/api/plugins/install-plan', {
                method: 'POST',
                headers,
                body: JSON.stringify(packageFixture.body)
            })))
            expect(planResponses.map((response) => response.status)).toEqual([200, 200, 200])
            const planIds = await Promise.all(planResponses.map(async (response) => ((await response.json()) as { planId: string }).planId))
            expect(readdirSync(tempRoot)).toHaveLength(1)

            const executeStatuses = await Promise.all(planIds.map(async (planId) => (await app.request(`/api/plugins/install-plan/${planId}/execute`, {
                method: 'POST',
                headers
            })).status))
            expect(executeStatuses.filter((status) => status === 200)).toHaveLength(1)
            expect(executeStatuses.filter((status) => status === 404)).toHaveLength(2)
            expect(readdirSync(tempRoot)).toHaveLength(0)
            expect(installRequests).toHaveLength(1)
        } finally {
    ...
    it('rejects oversized install-plan package payloads before temp storage', async () => {
        const tempRoot = mkdtempSync(join(tmpdir(), 'hapi-plugin-install-plan-size-test-'))
        const packageFixture = makeTgzPackage('com.example.large')
        try {
            const app = createInstallPlanRouteApp({
                manager: { listPlugins: () => [] } as never,
                installPlanTempDir: tempRoot,
                maxInstallPlanPackageBytes: 1
            })
            const response = await app.request('/api/plugins/install-plan', {
                method: 'POST',
                headers: { authorization: `Bearer ${await token()}`, 'content-type': 'application/json' },
                body: JSON.stringify(packageFixture.body)
            })
