...
function hostMatchesCertificate(host: string, cert: PeerCertificate): boolean {
    ...
    const commonNames = Array.isArray(cert.subject?.CN)
        ? cert.subject.CN
    ...
    if (hostIsIp) {
        return commonNames.some((commonName) => commonName === host)
    }

    return commonNames.some((commonName) => dnsNameMatchesHost(host, commonName))
}
