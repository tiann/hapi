...
describe('buildVisibleChatBlocks', () => {
    ...
    it.each(['command_actions', 'commandActions'])('groups shared Codex commands and patches with legacy tools (%s)', (actionsKey) => {
        ...
        const visible = buildVisibleChatBlocks(tools, { hasMoreMessages: false, groupingMode: 'grouped' })

        expect(tools.every((tool) => isEligibleForToolGrouping(tool, 'grouped'))).toBe(true)
        expect(visible).toHaveLength(1)
    ...
    it.each([
        ...
        for (const actions of [
            ...
            const visible = buildVisibleChatBlocks([...before, userShell, ...after], { hasMoreMessages: false, groupingMode: 'grouped' })

            expect(isEligibleForToolGrouping(userShell, 'grouped')).toBe(false)
            expect(visible).toHaveLength(3)
