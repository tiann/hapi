export default {
  ...
  'runner.skew.banner.title': '机器 {host} 上的运行器版本过旧',
  'runner.skew.banner.body': '此 Hub 需要机器 {host} 上更新的 HAPI CLI（当前 {version}）。请在该机器上升级 CLI，然后重启其运行器（`systemctl --user restart hapi-runner` 或 `hapi runner start`）。在运行器报告所需能力之前，此横幅会一直显示。',
  'runner.skew.banner.bodyUnknownVersion': '此 Hub 需要机器 {host} 上更新的 HAPI CLI。请在该机器上升级 CLI，然后重启其运行器（`systemctl --user restart hapi-runner` 或 `hapi runner start`）。在运行器报告所需能力之前，此横幅会一直显示。',
  'runner.skew.banner.summaryTitle': '{count} 台机器的运行器版本过旧',
  'runner.skew.banner.summaryBody': '机器是注册表中的主机；运行器是可自升级的守护进程。可对每台机器点 Upgrade，或等待自动尝试。可最小化或 snooze 本横幅以操作下方会话。',
  'runner.skew.banner.autoProblemTitle': '{count} 个运行器需要处理',
  'runner.skew.banner.autoProblemBody': '自动升级无法修复这些主机。请在机器上手动更新 CLI，或对受监督（仅重建）主机使用 Restart。',
  'runner.skew.banner.minimized': '{count} 台机器运行器过旧：{hosts} — 点按展开',
  'runner.skew.banner.minimize': '最小化',
  'runner.skew.banner.dismissTemp': '暂隐 1 小时',
  'runner.skew.banner.upgrade': '升级',
  'runner.skew.banner.upgrading': '正在升级…',
  ...
  'runner.skew.banner.restartNeedsSupervisor': 'Restart（停止）需要 HAPI_RUNNER_SUPERVISED=1，以便外部进程管理器重新拉起。普通主机请用 Upgrade 完成交接重启。',
  ...
  'runner.skew.banner.handoffDisabledHint': '（仅重建 — 已禁用 Upgrade；Restart 需要 HAPI_RUNNER_SUPERVISED=1）',
  'runner.skew.banner.upgradeNeedsHandoff': '该主机已退出版本交接（soup/仅重建）。请重建 soup 或清除 HAPI_DISABLE_VERSION_HANDOFF。若要用 Restart，请同时设置 HAPI_RUNNER_SUPERVISED=1。',
  'runner.skew.banner.upgradeNeedsManual': '该运行器过旧，无法从 Hub 自升级。请在该主机上手动安装更新的 HAPI CLI，然后重启运行器。',
  'runner.skew.banner.legacyRunnerHint': '（旧版运行器 — 已禁用 Upgrade；请在主机上更新 CLI）',
  ...
  'toast.runnerUpgrade.success.body': '{host} 已与 Hub 版本一致',
  'toast.runnerUpgrade.failed.title': '运行器升级失败',
  ...
  'settings.runnerMgmt.linkHint': '每台机器上的运行器如何跟随 Hub 版本',
  'settings.runnerMgmt.body': '机器是注册表中的主机；运行器是负责拉起会话与自升级的守护进程。本 Hub 可以让这些运行器保持与自身相同的 HAPI CLI 版本。当 Hub 升级时，运行器会自行更新以匹配 — 无需 SSH。大多数人无需更改此设置。',
  'settings.runnerMgmt.policyLabel': '当某台机器的运行器版本与 Hub 不一致时',
  'settings.runnerMgmt.policySilent': '不提醒',
  'settings.runnerMgmt.policySilentHint': '忽略差异',
  'settings.runnerMgmt.policyAlert': '提醒',
  'settings.runnerMgmt.policyAlertHint': '显示横幅；手动升级',
  'settings.runnerMgmt.policyAuto': '自动升级',
  'settings.runnerMgmt.policyAutoHint': '自动升级；仅当主机无法升级时提醒',
  'settings.runnerMgmt.optOutLabel': '想让某个运行器版本高于 Hub？',
  'settings.runnerMgmt.optOutBody': '较新的运行器不会被降级；自动升级只会推进落后于 Hub 的运行器。可用“不提醒/提醒”控制集群通知。HAPI_DISABLE_VERSION_HANDOFF=1 仅用于 soup/仅重建的 Hub 主机（普通舰队运行器不要设）。若 systemd/pm2 负责重启，请设置 HAPI_RUNNER_SUPERVISED=1 以便网页 Restart 安全可用。在 Hub 上设置 HAPI_UPGRADE_CHANNEL=off 会禁用所有升级。',
  ...
  'settings.voice.noTranscriptionProvider': 'Hub 尚未配置转录提供商 — 请在下方添加（或在可用时使用浏览器本地识别）。',
