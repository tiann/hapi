export default {
  ...
  'runner.skew.banner.binaryUpdatedHint': '(newer CLI on disk — restart may pick it up)',
  'runner.skew.banner.autoUpgradeHint': 'Opt-in auto-restart when a newer CLI is already on disk: set HAPI_AUTO_UPGRADE_RUNNERS=1 (or autoUpgradeRunners: true in hub settings.json). Off by default. Does not push packages to remote machines.',
