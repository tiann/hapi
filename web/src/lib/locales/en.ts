export default {
  ...
  'runner.skew.banner.title': 'Outdated runner on machine {host}',
  'runner.skew.banner.body': 'This hub needs a newer HAPI CLI on machine {host} (running {version}). Upgrade the CLI on that machine, then restart its runner (`systemctl --user restart hapi-runner` or `hapi runner start`). This banner stays until the runner reports the required capabilities.',
  'runner.skew.banner.bodyUnknownVersion': 'This hub needs a newer HAPI CLI on machine {host}. Upgrade the CLI on that machine, then restart its runner (`systemctl --user restart hapi-runner` or `hapi runner start`). This banner stays until the runner reports the required capabilities.',
  'runner.skew.banner.summaryTitle': '{count} machine(s) with outdated runners',
  'runner.skew.banner.summaryBody': 'A machine is a host in the registry; its runner is the daemon that can self-upgrade. Use Upgrade per machine, or wait for auto. Minimize or snooze this banner to reach sessions underneath.',
  'runner.skew.banner.autoProblemTitle': '{count} runner(s) need attention',
  'runner.skew.banner.autoProblemBody': 'Auto-upgrade cannot fix these hosts. Update the CLI on the machine, or Restart a supervised (rebuild-only) host.',
  'runner.skew.banner.minimized': '{count} machine(s) with outdated runners: {hosts} — tap to expand',
  'runner.skew.banner.minimize': 'Minimize',
  'runner.skew.banner.dismissTemp': 'Snooze 1h',
  'runner.skew.banner.upgrade': 'Upgrade',
  'runner.skew.banner.upgrading': 'Upgrading…',
  ...
  'runner.skew.banner.restartNeedsSupervisor': 'Restart (stop) requires HAPI_RUNNER_SUPERVISED=1 so an external supervisor relaunches the runner. Prefer Upgrade for a handoff relaunch.',
  ...
  'runner.skew.banner.handoffDisabledHint': '(rebuild-only — Upgrade disabled; Restart needs HAPI_RUNNER_SUPERVISED=1)',
  'runner.skew.banner.upgradeNeedsHandoff': 'This machine opted out of version handoff (soup/rebuild-only hub host). Rematerialize soup or clear HAPI_DISABLE_VERSION_HANDOFF. For Restart under systemd/pm2, also set HAPI_RUNNER_SUPERVISED=1. Ordinary fleet runners must not set the handoff opt-out.',
  'runner.skew.banner.upgradeNeedsManual': 'This machine\'s runner is too old to self-upgrade from the hub. Install a newer HAPI CLI on that machine, then restart the runner.',
  'runner.skew.banner.legacyRunnerHint': '(legacy runner — Upgrade disabled; update CLI on the machine)',
  'runner.skew.banner.handoffHint': 'Fleet upgrade: hub detects npm vs source install and upgrades remote runners accordingly (npm registry or hub-built artifact). Upgrade never SSHs; the runner on each machine installs on itself. Set HAPI_UPGRADE_CHANNEL=off to disable.',
  ...
  'toast.runnerUpgrade.success.body': '{host} is now on the hub version',
  'toast.runnerUpgrade.failed.title': 'Runner upgrade failed',
  'toast.runnerUpgrade.failed.body': '{host}: {message}',
  ...
  'settings.runnerMgmt.linkHint': 'How each machine\'s runner tracks the hub version',
  'settings.runnerMgmt.body': 'A machine is a host in the registry; its runner is the daemon that handles spawn and self-upgrade. This hub can keep those runners on its own HAPI CLI version. When the hub upgrades, runners self-update to match — no SSH. Most people never need to change this.',
  'settings.runnerMgmt.policyLabel': 'When a machine\'s runner drifts from the hub version',
  'settings.runnerMgmt.policySilent': 'No alert',
  'settings.runnerMgmt.policySilentHint': 'Ignore drift',
  'settings.runnerMgmt.policyAlert': 'Alert',
  'settings.runnerMgmt.policyAlertHint': 'Show a banner; upgrade by hand',
  'settings.runnerMgmt.policyAuto': 'Auto-upgrade',
  'settings.runnerMgmt.policyAutoHint': 'Upgrade for me; alert only if a host cannot',
  'settings.runnerMgmt.optOutLabel': 'Want a runner ahead of the hub?',
  'settings.runnerMgmt.optOutBody': 'Newer runners are left untouched; auto-upgrade only advances machines whose runners trail the hub. Use No alert or Alert for fleet-wide notifications. HAPI_DISABLE_VERSION_HANDOFF=1 is for soup/rebuild-only hub hosts (not ordinary fleet runners). Set HAPI_RUNNER_SUPERVISED=1 when systemd/pm2 owns restart so the web Restart button is safe. HAPI_UPGRADE_CHANNEL=off on the hub disables all upgrades.',
  ...
  'settings.voice.noTranscriptionProvider': 'No hub transcription provider yet — add one below (or use Browser on-device if available).',
