...
export const knownTools: Record<string, {
    ...
    CodexSpawnAgent: {
        ...
        subtitle: (opts) => {
            ...
            const effort = getInputStringAny(opts.input, ['reasoning_effort'])
    ...
    BashOutput: {
        ...
        subtitle: (opts) => {
            const text = typeof opts.result === 'string'
                ? opts.result
                : getInputStringAny(opts.result, ['stdout', 'stderr', 'output', 'text', 'content'])
            return text ? truncate(text.replace(/\r?\n/g, ' ↩ '), 120) : null
        },
        minimal: true
    },
    KillBash: {
        icon: () => <TerminalIcon className={DEFAULT_ICON_CLASS} />,
        title: () => 'Stop command',
        subtitle: (opts) => getInputTextAny(opts.input, ['target', 'pid', 'process_id']) ?? null,
        minimal: true
    },
    TodoRead: {
    ...
    EnterWorktree: {
        ...
        subtitle: (opts) => getInputStringAny(opts.input, ['path', 'worktreePath', 'worktree_path']) ?? null,
        minimal: true
    },
    shell_command: {
        icon: () => <TerminalIcon className={DEFAULT_ICON_CLASS} />,
