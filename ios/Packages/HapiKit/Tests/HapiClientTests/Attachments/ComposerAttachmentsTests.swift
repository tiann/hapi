...
struct ComposerAttachmentsTests {
    ...
    @Test func addUploadsImmediatelyWithTheExactBase64PayloadAndSettlesReady() async throws {
        ...
        let expectedThumbnailBase64 = Data([1, 2]).base64EncodedString()
        let uploads = await harness.performer.bodies(pathSuffix: "/api/sessions/sess-att/upload")
        #expect(uploads == [
            "{\"content\":\"\(expectedBase64)\",\"filename\":\"shot.jpg\",\"mimeType\":\"image/jpeg\",\"thumbnailContent\":\"\(expectedThumbnailBase64)\",\"thumbnailMimeType\":\"image/jpeg\"}"
        ])
    }
    ...
    @Test func durableUploadKeepsOnlyTheAttachmentIdAndThumbnailData() async throws {
        let harness = try TrayHarness()
        await harness.performer.scriptUpload(
            json: #"{"success":true,"attachmentId":"durable-1","thumbnailAvailable":true}"#
        )
        ...
        let expectedThumbnail = Data([9, 9]).base64EncodedString()
        let uploads = await harness.performer.bodies(pathSuffix: "/upload")
        #expect(uploads == [
            "{\"content\":\"\(expectedContent)\",\"filename\":\"shot.jpg\",\"mimeType\":\"image/jpeg\",\"thumbnailContent\":\"\(expectedThumbnail)\",\"thumbnailMimeType\":\"image/jpeg\"}"
        ])
    }
