...
**Status (2026-06-15 18:27):** v2.0 SHIPPED upstream as PR [tiann/hapi#896](https://github.com/tiann/hapi/pull/896) (issue [#893](https://github.com/tiann/hapi/issues/893), v2.1 tracking issue [#894](https://github.com/tiann/hapi/issues/894)). Branch `heavygee:feat/scratchlist-v2`, +2605/-11 across 24 files. All gates green at submission. **Soup integration done 2026-06-15:** layered as `soup/scratchlist-v2-v11` with two operator-local soup-only fixup commits (schema renumber 10->11 to coexist with `feat/companion-fcm-push-api`'s v10 bump; SessionList.tsx import dedup against `feat/session-list-rich-tooltips`'s `relativeTime.ts`). Driver/integration tip `9b935338` has 12 layers merged. Web bundle swapped atomically. Two pre-existing upstream typecheck errors in `tunnel/tlsGate.ts:70` and `web/routes/guards.ts:42` (byte-identical to `upstream/main`, not blocking). Active driver swap (`hapi-use-driver`) and cross-device test still pending operator.

**Rotation/data-loss reframed (PR-discovery 2026-06-15):** the "HAPI session ID changes on hub bounce" issue is NOT a bug; it's intentional dedup ([tiann/hapi#448](https://github.com/tiann/hapi/issues/448)). The actual hazard is `mergeSessionData` in `hub/src/sync/sessionCache.ts` lines 682-813 transferring a fixed field list then cascade-deleting the old row, silently dropping FK-tied data (scratchlist entries among others). Filed as [tiann/hapi#920](https://github.com/tiann/hapi/issues/920) framed as a prerequisite for v2's durability promise and v2.1's consent flow. Cross-linked on #893 and #894 with scope-expansion notes; #917 (reopen creates dead session) is the same gap from a different angle.

**Original status (2026-06-13 17:41):** v2.0 SHIPPED to upstream PR; sibling peer 7b422b92 added `todos`/`teamState`/versioned `metadata`/versioned `agentState` to the same `SessionPatchSchema` for Fix B' (#895/#897); v2 added `scratchlistUpdatedAt?: number` adjacent.
