...
Status: **All phases DONE 2026-06-05; final orphan cleared 2026-06-06.** Phase 1 (Option B) 2026-06-01; Phase 2 (drain) executed opportunistically 2026-06-04 through 2026-06-05 across two cleanup passes (android-agent triage + operator cleanup session); Phase 3 (Option C) executed 2026-06-05 in an impatient hub-down window (~1 min downtime, single active session 24f3ec91 was cut). All worktrees now live under canonical `~/coding/hapi/{driver,upstream,active,worktrees/*}`. `hapi-main` symlink removed (hard-cut). `~/coding/hapi-worktrees/voice-selection-all-backends-worktrees/0531-4033` was deleted 2026-06-06 after confirmation that the voice-selection branch had merged upstream as PR #743 and the orphan dir was empty. Top-level neighbors of `~/coding/hapi/` are now only the two intentional independents: `hapi-companion` (separate Android product repo) and `hapi-monitor` (npm-installed tool with own git origin). Operator re-affirmed 2026-06-06 against further consolidation (e.g. moving the repo into a subfolder so `~/coding/hapi/` becomes a pure container) — the plan's constraint §2 line "`~/coding/hapi/` IS the canonical fork mirror checkout; cannot become a pure parent dir without moving `.git`" still stands.
