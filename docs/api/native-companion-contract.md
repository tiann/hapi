...
| Key | Example | Purpose |
|-----|---------|---------|
| `type` | `ready` | `ready`, `permission-request`, `input-request`, `task-notification`, `model-error` |
...
| `severity` | `info` | `info` (ready/input), `warning` (permission), `success` / `error` (task), `error` (`model-error`) |
