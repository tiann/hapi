...
| `POST /api/sessions/:id/model` | `{model: string \| {provider, modelId} \| null, effort?: string \| null}` | Flavors with `supportsModelChange` (not `dsh`); remote-only for codex/cursor/grok. `effort` is optional and **Claude-only**: it is applied in the same config write, so a model change that invalidates the current effort clears it atomically. Other flavors must use the separate `/effort` route because their agents do not apply the pair as one operation (400 otherwise) |
