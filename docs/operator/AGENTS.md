...
- `sudo systemctl stop|restart|kill|disable|mask hapi-{hub,runner,runner-watchdog}.service` — naked, no patient drain. **Blocked at three layers since 2026-06-11, agent-agnostic** — applies to Cursor, Claude Code, Codex, Gemini, ad-hoc operator shells, anything: (1) `/usr/local/sbin/systemctl` wrapper on sudo's `secure_path` catches every `sudo systemctl ...` invocation including `sudo bash -c 'systemctl ...'` shell-wraps; (2) `/etc/sudoers.d/hapi-protect` `!`-rule catches absolute-path bypass `sudo /bin/systemctl ...`; (3) Cursor preToolUse hook preempts the agent's tool call before sudo runs. Allowed: `start`, `status`, `is-active`, watchdog's `restart hapi-runner.service` — recovery and read-only ops. Bypass: `HAPI_OPERATOR_SYSTEMCTL_OVERRIDE=1 sudo -E systemctl ...`.
